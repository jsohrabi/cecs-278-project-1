...
public abstract class Entity {
	...
	/**
	 * Adds the parameterized int to the Entity's current HP. If the sum of the Entity's max HP and the parameterized int exceed their max HP, their current HP is set to the max HP.
	 * 
	...
	/**
	 * Subtracts the parameterized int from the Entity's max HP. If the resulting HP is less than 0, the current HP is set to 0.
	 * 
	...
	/**
	 * Decreases the Entity's max HP by the parameterized int. If the result is less than or equal to 0, the max HP is set to 1.
	 * 
	...
	/**
	 * Displays the Entity's name, level, and HP.
	 */
	public void display() {
		System.out.println( this.getName() + " Lvl: " + this.getLevel() );			//Write name, level of Entity to console.
		System.out.println( "HP: " + this.getHP() + "/" + this.getMaxHP() );		//Write Entity HP status to console.
	}
}
