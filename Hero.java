...
public class Hero extends Entity implements Magical {
	...
	/**
	 * Displays the name, health, held inventory, and held currency of the hero.
	 */
	public void display() {
		super.display();								//Calls display() from the Entity superclass.
		System.out.println( "$: " + this.getGold() );	//Writes the Hero's current gold to the console.
		this.displayItems();							//Write the Hero's inventory to the console.
	}
	
	/**
	 * Displays the hero's currently held inventory.
	 */
	public void displayItems() {
		System.out.println( "Inventory:" );
		if ( this.getNumItems() == 0 ) {				//If the Hero has no inventory items, write it to the console.
			System.out.println( "Nothing in inventory" );
		} else {										//If the Hero has inventory items, loop through each Item in their inventory and write their names to the console.
	...
	public Item removeItem( String n ) {
		for( Item i: this.items ) {									//Loop through each Item in the items ArrayList.
			if ( i.getName() == n ) {								//If the Item's name matches the String parameter n:
				if ( !i.getName().equals( "Health Potion" ) ) {		//If the Item is not a Health Potion:
					if ( ( this.getHP() - i.getValue() ) <= 0 ) {	//If removing the Item would set the Hero's HP to less than or equal to 0, warn the player that they cannot remove the Item.
						System.out.println( "Removing this item would kill you." );
	...
	public void attack( Entity e ) {
		...
		String spellUsed = "";
		int damage = 0;															//Used to store damage dealt by the attack.
		boolean turnDone = false;												//Indicates whether the Hero is done with their turn of combat.
		final String ATTACK_MENU = "1. Physical Attack\n2. Magical Attack";		//String for showing different Hero attack options.
		
		//Do while the Hero is not done with their combat turn.
		do {
			System.out.println( ATTACK_MENU );									//Write attack options to console.
			if ( this.hasPotion() )												//If the Hero has a Health Potion, show them the option to use it.
				System.out.println( "3. Use Potion" );
			switch( CheckInput.getIntRange( 1, 3 ) ) {							//Take an input in the range of [1,3] from the user.
				//If 1 was input by the player.
				case 1:
					damage = ( random.nextInt( 4 ) + 1 ) * this.getLevel();		//Generates a Random number in the range of [1,4] and multiplies it by Hero's level to get attack damage.
					turnDone = true;											//Sets the Hero's turn as finished.
					System.out.println( getName() + " attacks the " + e.getName() + " for " + damage + " damage." );		//Write the attack to console.
					e.takeDamage( damage );										//Deal damage to Hero's attack target.
					break;														//Break out of this switch statement.
				//If 2 was input by the player.
				case 2:
					System.out.println( MAGIC_MENU );							//Write the MAGIC_MENU to the console.
						switch ( CheckInput.getIntRange(1, 3) ) {				//Take an input in the range of [1,3] from the user.
							//If 1 was input.
							case 1:
								spellUsed = "Magic Missile";
								damage = magicMissile();						//Gets damage from MagicMissile().
								turnDone = true;								//Sets Hero's turn as finished.
								break;											//Break out of this switch statement.
							case 2:
								spellUsed = "Fireball";
								damage = fireball();							//Gets damage from Fireball().
								turnDone = true;								//Sets Hero's turn as finished.
								break;											//Break out of this switch statement.
							case 3:
								spellUsed = "Thunderclap";						
								damage = thunderclap();							//Gets damage from MagicMissile().
								turnDone = true;								//Sets Hero's turn as finished.
								break;											//Break out of this switch statement.
						}
					System.out.println( getName() + " attacks the " + e.getName() + " with a " + spellUsed + " for " + damage + " damage." );	//Write the attack to the console.
					e.takeDamage( damage );										//Deal damage to the Enemy.
					break;														//Break out of this switch statement.
				//If 3 was input by the player.
				case 3:
					if ( this.hasPotion() ) {									//If the Hero has a Health Potion in their inventory:
						this.removeItem( "Health Potion" );						//Remove the Health Potion from their inventory.
						this.heal(25);											//Heal the Hero for 25 HP.
						System.out.println( "You were healed by your health potion." );
						turnDone = true;										//Set the Hero's turn as finished.
						break;													//Break out of this switch statement.
					} else {
						System.out.println( "Invalid Range" );
						turnDone = false;										//Do not set the Hero's turn as finished.
						break;													//Break out of this switch statement.
					}
			}
		} while( !turnDone );
